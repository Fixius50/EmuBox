...
impl BiosService {
    pub fn get_bios_requirements() -> Result<BiosStatus, EmuBoxError> {
        let manifest: BiosManifest =
            serde_json::from_str(include_str!("../../../data/bios/bios-manifest.json"))
                ...
                if let Some(path) = &found {
                    for (program, expected) in [
                        ...
                        if let Some(expected) = expected {
                            let digest = super::host_command::output(
                                program,
...
mod tests {
    ...
    fn shipped_requirements_are_not_empty() {
        let manifest =
            serde_json::from_str(include_str!("../../../data/bios/bios-manifest.json")).unwrap();
