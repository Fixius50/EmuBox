use crate::{
    errors::EmuBoxError,
    models::{GamepadDevice, GamepadStatus},
};
...
fn bit_count(bitmap: &str) -> Option<usize> {
    bitmap.split_whitespace().try_fold(0, |total, chunk| {
        u64::from_str_radix(chunk, 16)
            .ok()
            .map(|value| total + value.count_ones() as usize)
    })
}

pub fn devices() -> Result<Vec<GamepadDevice>, EmuBoxError> {
    let entries = fs::read_dir("/sys/class/input")
        .map_err(|error| EmuBoxError::HardwareUnavailable(error.to_string()))?;
    ...
    for entry in entries {
        ...
        devices.push(GamepadDevice {
            index,
            id: format!("/dev/input/{name}"),
            name: read(&device.join("name")).unwrap_or(name),
            connected: true,
            vendor_id: read(&device.join("id/vendor")),
            product_id: read(&device.join("id/product")),
            buttons_count: read(&device.join("capabilities/key"))
                .and_then(|value| bit_count(&value)),
            axes_count: read(&device.join("capabilities/abs")).and_then(|value| bit_count(&value)),
            has_vibration: read(&device.join("capabilities/ff"))
                .and_then(|value| bit_count(&value))
                .map(|count| count > 0),
    ...
    if let Some(primary) = devices.first_mut() {
...
pub fn status() -> Result<GamepadStatus, EmuBoxError> {
    ...
    Ok(GamepadStatus {
        ...
        primary_device_index: devices.first().map(|device| device.index),
        devices,
    })
}
