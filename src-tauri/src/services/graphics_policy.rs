pub fn vendor_from_text(text: &str) -> &'static str {
    ...
    let vendors: &[(&str, &[&str])] = &[
        ("virtual", &["vmware", "vmwgfx", "virtualbox", "virtio", "qxl", "virgl", "svga3d", "venus"]),
        ...
        ("qualcomm", &["qualcomm", "adreno", "msm", "freedreno", "5143"]),
        ("apple", &["apple", "asahi", "agx", "106b"]),
    ];
    vendors.iter().find(|(_, identifiers)| identifiers.iter().any(|identifier| words.contains(identifier)))
        .map(|(vendor, _)| *vendor).unwrap_or("unknown")
}
...
pub fn hardware_vulkan(summary: &str) -> Option<(String, Option<String>)> {
    ...
    for line in summary.lines().chain(std::iter::once("GPU999:")) {
        ...
        if line.starts_with("GPU") && line.ends_with(':') {
            if hardware && name.as_deref().is_some_and(|renderer| !is_software_renderer(renderer)) {
...
pub fn opengl_renderer(summary: &str) -> Option<String> {
    let renderers: Vec<_> = summary.lines().filter_map(|line| {
        ...
        match key {
            "OpenGL core profile renderer" | "OpenGL compatibility profile renderer"
            | "OpenGL ES profile renderer" | "OpenGL renderer string" if !value.trim().is_empty() => Some(value.trim().to_string()),
            _ => None,
        }
    }).collect();
    renderers.iter().find(|renderer| !is_software_renderer(renderer)).or_else(|| renderers.first()).cloned()
}
