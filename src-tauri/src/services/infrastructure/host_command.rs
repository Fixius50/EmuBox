...
pub fn output_with_timeout(program: &str, arguments: &[&str], duration: &str) -> Result<String, EmuBoxError> {
    ...
    if !result.status.success() {
        if matches!(result.status.code(), Some(124 | 137)) {
            return Err(EmuBoxError::ProcessFailed(format!("{program}: tiempo limite de {duration} agotado")));
        }
...
mod tests {
    ...
    fn bounded_commands_report_success_failure_and_timeout() {
        assert_eq!(super::output_with_timeout("printf", &["version"], "1s").unwrap(), "version");
        assert!(super::output_with_timeout("false", &[], "1s").is_err());
        let error = super::output_with_timeout("sh", &["-c", "while :; do :; done"], "0.1s").unwrap_err();
