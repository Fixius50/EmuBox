...
fn validate_inner(
    ...
    if executable && header[..size].starts_with(b"#!") {
        ...
        let interpreter = if Path::new(interpreter)
            .file_name()
            .is_some_and(|name| name == "env")
        {
            words
                ...
                .ok_or("Unsupported env wrapper; configure an explicit interpreter")?
        } else {
            interpreter
        };
        let resolved =
            resolve_executable(interpreter).ok_or("Wrapper interpreter not installed")?;
    ...
    if host == Architecture::Unsupported || binary != host {
        return Err(format!(
            "Unsupported architecture: host={}, binary={}, path={}",
            host.as_str(),
            binary.as_str(),
            path.display()
        ));
...
pub fn resolve_core(core: &str) -> Option<PathBuf> {
    ...
    [
        PathBuf::from(core),
        Path::new("/usr/lib/libretro").join(name),
...
mod tests {
    ...
    fn binaries_and_cores_matrix() {
        let directory =
            std::env::temp_dir().join(format!("emubox-binary-test-{}", std::process::id()));
        ...
        for (machine, architecture) in [(62u16, Architecture::X86_64), (183, Architecture::Aarch64)]
        {
            ...
            for host in [
                ...
                assert_eq!(
                    validate_binary(&path, host, true).is_ok(),
                    host == architecture
                );
                assert_eq!(
                    validate_binary(&path, host, false).is_ok(),
                    host == architecture
                );
            }
        ...
        assert!(
            validate_binary(&directory.join("missing"), Architecture::current(), true).is_err()
        );
