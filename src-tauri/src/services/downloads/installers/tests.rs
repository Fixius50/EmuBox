...
fn prepared_windows_requires_pe_and_explicit_selection() {
    let root = std::env::temp_dir().join(format!("emubox-inno-candidates-{}", std::process::id()));
...
fn rejects_links_in_prepared_content() {
    let root = std::env::temp_dir().join(format!("emubox-installer-links-{}", std::process::id()));
...
fn real_rpcs3_starts_in_installation_sandbox() {
    let root = std::env::temp_dir().join(format!("emubox-rpcs3-sandbox-{}", std::process::id()));
    fs::create_dir_all(&root).unwrap();
    ...
    .to_string();
    assert!(
        failed.contains("firmware") || failed.contains("no confirmo") || failed.contains("rechazo"),
        "{failed}"
    );
