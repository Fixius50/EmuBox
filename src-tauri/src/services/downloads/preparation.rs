...
pub fn prepare(
    ...
    if let Some(kind) = crate::services::installer_preparation::kind(&header[..count]) {
        return crate::services::installer_preparation::prepare(&files[0], root, kind, control);
    }
    if crate::services::archive_preparation::supported_signature(&header[..count]) {
        return crate::services::archive_preparation::extract(&files[0], root, control);
    }
    if count < 4 || !header.starts_with(b"PK\x03\x04") {
        return Ok(files.to_vec());
    }
    ...
    let mut archive =
        zip::ZipArchive::new(fs::File::open(&files[0]).map_err(io_error)?).map_err(io_error)?;
    if archive.len() > 100_000 {
        return Err(EmuBoxError::InvalidConfiguration(
            "ZIP supera el limite de archivos".into(),
        ));
    }
    let mut total = 0u64;
    ...
    for index in 0..archive.len() {
        ...
        let relative = entry.enclosed_name().ok_or_else(|| {
            EmuBoxError::InvalidConfiguration("ZIP contiene una ruta fuera del paquete".into())
        })?;
        if entry
            .unix_mode()
        ...
        {
            return Err(EmuBoxError::InvalidConfiguration(
                "ZIP contiene enlaces simbolicos".into(),
            ));
        }
        if relative
            .components()
        ...
        {
            return Err(EmuBoxError::InvalidConfiguration(
                "ZIP contiene una ruta no segura".into(),
            ));
        }
        total = total
            .checked_add(entry.size())
            .ok_or_else(|| EmuBoxError::InvalidConfiguration("Tamano ZIP desbordado".into()))?;
        if entry.size() > 1024 * 1024 && entry.size() / entry.compressed_size().max(1) > 1000 {
            return Err(EmuBoxError::InvalidConfiguration(
                "ZIP excede la relacion de expansion permitida".into(),
            ));
        }
        if total > 100 * 1024 * 1024 * 1024 {
            return Err(EmuBoxError::InvalidConfiguration(
                "ZIP excede 100 GiB descomprimidos".into(),
            ));
        }
        ...
        loop {
            ...
            if written > entry.size() {
                return Err(EmuBoxError::InvalidConfiguration(
                    "ZIP excede tamano declarado".into(),
                ));
            }
...
pub fn verify(
    ...
    for path in files {
        if !fs::canonicalize(path).map_err(io_error)?.starts_with(&root)
        ...
        {
            return Err(EmuBoxError::InvalidConfiguration(
                "Artefacto fuera de la zona de descarga".into(),
            ));
        }
    }
    if let Some(expected) = checksum {
        ...
        {
            return Err(EmuBoxError::InvalidConfiguration(
                "Checksum no compatible: se requiere SHA-256 hexadecimal".into(),
            ));
        }
        if files.len() != 1 {
            return Err(EmuBoxError::InvalidConfiguration(
                "Checksum de paquete ambiguo para multiples archivos".into(),
            ));
        }
        let mut file = fs::File::open(&files[0]).map_err(io_error)?;
        ...
        loop {
            if control.interrupted() {
                return Err(EmuBoxError::ProcessFailed(
                    "Verificacion interrumpida".into(),
                ));
            }
        ...
        if !format!("{:x}", hasher.finalize()).eq_ignore_ascii_case(expected) {
            return Err(EmuBoxError::InvalidConfiguration(
                "Checksum SHA-256 no coincide; archivo no publicado".into(),
            ));
        }
    }
    Ok(())
...
pub fn launch_target(platform: &str, files: &[PathBuf]) -> Option<PathBuf> {
    ...
    let specification = crate::services::game_service::PLATFORM_SPECS
        .iter()
        .find(|specification| specification.id == platform)?;
    let candidates: Vec<_> = files
        ...
        .filter(|path| {
            ...
            !matches!(
                extension.as_str(),
                "zip" | "7z" | "rar" | "pkg" | "exe" | "bin" | "cue" | "m3u"
            ) && specification.extensions.contains(&extension.as_str())
        })
        .collect();
...
mod tests {
    ...
    fn archives_and_installers_are_not_marked_playable() {
        ...
        assert_eq!(
            launch_target("ps2", &["game.iso".into()]),
            Some("game.iso".into())
        );
        assert_eq!(
            launch_target("ps2", &["one.iso".into(), "two.iso".into()]),
            None
        );
    }
    ...
    fn zip_preparation_preserves_relative_paths() {
        ...
        zip.start_file("folder/game.chd", zip::write::SimpleFileOptions::default())
            .unwrap();
        zip.write_all(b"local test data").unwrap();
        zip.finish().unwrap();
