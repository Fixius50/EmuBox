...
pub fn remember(connection: &Connection, record: &CacheRecord) -> Result<(), EmuBoxError> {
    ...
    connection
        .execute(
            "INSERT INTO manifest_http_cache VALUES (?1, ?2, ?3, ?4, ?5, ?6)
    ...
    connection
        .execute(
            "DELETE FROM manifest_fetch_failures WHERE url=?1",
            params![record.url],
        )
        .map_err(storage)?;
    Ok(())
...
pub fn fetch(connection: &Connection, url: &str, now: u64) -> Result<FetchResult, EmuBoxError> {
    initialize(connection)?;
    let retry_after: Option<u64> = connection
        .query_row(
            "SELECT retry_after FROM manifest_fetch_failures WHERE url=?1",
            params![url],
            |row| row.get(0),
        )
        .optional()
        .map_err(storage)?;
    if retry_after.is_some_and(|retry| retry > now && retry - now <= 15 * 60) {
    ...
    if result.is_err() {
        connection
            .execute(
                "INSERT INTO manifest_fetch_failures VALUES (?1, ?2)
            ON CONFLICT(url) DO UPDATE SET retry_after=excluded.retry_after",
                params![url, now + 15 * 60],
            )
            .map_err(storage)?;
    }
    result
...
fn fetch_current(connection: &Connection, url: &str, now: u64) -> Result<FetchResult, EmuBoxError> {
    ...
    if previous
        .as_ref()
        .is_some_and(|record| now >= record.checked && now - record.checked < REFRESH_SECONDS)
    {
        return Ok(FetchResult::Fresh);
    }
    ...
    if !matches!(parsed.scheme(), "http" | "https") {
        return Err(EmuBoxError::InvalidConfiguration(
            "Solo se aceptan manifiestos HTTP/HTTPS".into(),
        ));
    }
    ...
    let response = request
        ...
        .map_err(|error| {
            EmuBoxError::Unknown(format!("No se pudo comprobar el manifiesto: {error}"))
        })?;
    let etag = response
        .headers()
        .get(header::ETAG)
    ...
    if response.status() == StatusCode::NOT_MODIFIED {
        let mut record = previous.ok_or_else(|| {
            EmuBoxError::InvalidConfiguration("HTTP 304 sin catalogo importado".into())
        })?;
        record.checked = now;
    ...
    if content.len() as u64 > MAX_BYTES {
        return Err(EmuBoxError::InvalidConfiguration(
            "El manifiesto supera 32 MiB".into(),
        ));
    }
    ...
    if unchanged {
        return Ok(FetchResult::Unchanged(cache));
    }
    let filename = parsed
        .path_segments()
        .and_then(|mut segments| segments.rfind(|segment| !segment.is_empty()))
        .map(str::to_owned);
    Ok(FetchResult::Changed {
        content,
        filename,
        cache,
    })
...
mod tests {
    ...
    fn conditional_requests_skip_fresh_and_unchanged_content() {
        ...
        assert!(matches!(
            fetch(&connection, &url, start + 1).unwrap(),
            FetchResult::Fresh
        ));
        let FetchResult::Unchanged(cache) =
            fetch(&connection, &url, start + REFRESH_SECONDS).unwrap()
        else {
            panic!("304");
        };
        remember(&connection, &cache).unwrap();
        let FetchResult::Unchanged(cache) =
            fetch(&connection, &url, start + 2 * REFRESH_SECONDS).unwrap()
        else {
            panic!("digest");
        };
        remember(&connection, &cache).unwrap();
        assert!(matches!(
            fetch(&connection, &url, start + 3 * REFRESH_SECONDS).unwrap(),
            FetchResult::Changed { .. }
        ));
        assert!(fetch(&connection, &url, start + 3 * REFRESH_SECONDS + 1).is_err());
        assert!(matches!(
            fetch(&connection, &url, start + 3 * REFRESH_SECONDS + 2).unwrap(),
            FetchResult::Fresh
        ));
