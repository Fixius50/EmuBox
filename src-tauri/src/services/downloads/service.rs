...
impl DownloadService {
    ...
    fn fetch_manifest(url_str: &str) -> Result<crate::services::manifest_cache::FetchResult, EmuBoxError> {
        let connection = DatabaseService::get_connection()?;
        crate::services::manifest_cache::fetch(&connection, url_str, chrono::Utc::now().timestamp().max(0) as u64)
    }

    fn import_fetched(fetched: crate::services::manifest_cache::FetchResult) -> Result<Vec<DownloadSource>, EmuBoxError> {
    ...
    fn import_manifest_cached(manifest: &str, source_name_fallback: Option<&str>, cache: Option<&crate::services::manifest_cache::CacheRecord>) -> Result<Vec<DownloadSource>, EmuBoxError> {
        ...
        let normalized: Vec<_> = entries.iter().filter_map(crate::services::manifest_service::normalize).collect();
    ...
    pub fn create_job(request: CreateDownloadRequest) -> Result<DownloadJob, EmuBoxError> {
        let _guard = JOB_CREATION.lock().map_err(crate::services::download_providers::io_error)?;
        ...
        let destination = crate::services::download_manager::content_root().join(&request.platform).join(&id);
        let mut conn = DatabaseService::get_connection()?;
        let transaction = conn.transaction().map_err(crate::services::download_providers::io_error)?;
        transaction.execute(
        ...
        transaction.execute("INSERT INTO download_execution(job_id,source_json,provider) VALUES (?1,?2,?3)", params![id,serde_json::to_string(&source).map_err(crate::services::download_providers::io_error)?,provider.as_str()]).map_err(crate::services::download_providers::io_error)?;
        transaction.commit().map_err(crate::services::download_providers::io_error)?;
    ...
    pub fn list_jobs() -> Result<Vec<DownloadJob>, EmuBoxError> {
        ...
        rows.collect::<Result<Vec<_>, _>>().map_err(crate::services::download_providers::io_error)
    }
    ...
    pub fn list_sources(game_id: &str) -> Result<Vec<crate::services::manifest_service::SourceOption>, EmuBoxError> {
        let conn = DatabaseService::get_connection()?;
        ...
        let rows = statement.query_map(params![game_id], |row| {
            ...
            Ok(crate::services::manifest_service::source_option(DownloadSource {
...
mod tests {
    ...
    fn manifest_cache_updates_only_changed_entries_and_keeps_user_state() {
        ...
        assert!(matches!(crate::services::manifest_cache::fetch(&reopened, url, 201).unwrap(), crate::services::manifest_cache::FetchResult::Fresh));
