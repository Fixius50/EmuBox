use super::{io_error, safe_name, DownloadProvider};
use crate::{
    errors::EmuBoxError,
...
async fn controlled<T>(
    ...
    control: &crate::models::TransferControl,
) -> Result<Option<T>, EmuBoxError> {
...
async fn transfer(
    ...
    let url = reqwest::Url::parse(request.uri)
        .map_err(|_| EmuBoxError::InvalidConfiguration("URI HTTP invalida".into()))?;
    if !matches!(url.scheme(), "http" | "https")
        || !url.username().is_empty()
        || url.password().is_some()
    {
        return Err(EmuBoxError::InvalidConfiguration(
            "HTTP requiere URL sin credenciales embebidas".into(),
        ));
    }
    ...
    let identity = fs::read(&identity_path)
        .ok()
        .and_then(|bytes| serde_json::from_slice::<ResumeIdentity>(&bytes).ok());
    let validator = identity
        .as_ref()
        .filter(|identity| identity.uri == request.uri)
    ...
    if offset > 0 {
        builder = builder
            ...
            .header(header::IF_RANGE, validator.unwrap());
    }
    let Some(mut response) = controlled(builder.send(), request.control).await? else {
        return Ok(TransferOutcome::Interrupted);
    };
    if !matches!(response.url().scheme(), "http" | "https") {
        return Err(EmuBoxError::InvalidConfiguration(
            "Redireccion a protocolo no permitido".into(),
        ));
    }
    if response.status() == StatusCode::RANGE_NOT_SATISFIABLE {
        let _ = fs::remove_file(&identity_path);
        return Err(EmuBoxError::IpcError(
            "HTTP 416: parcial obsoleto; reanudar reiniciara la transferencia".into(),
        ));
    }
    if !response.status().is_success() {
        return Err(EmuBoxError::IpcError(format!(
            "HTTP {}: fuente no accesible",
    ...
    let media = response
        ...
        .unwrap_or("")
        .to_ascii_lowercase();
    if media.contains("text/html") || media.contains("application/xhtml") {
        return Err(EmuBoxError::InvalidConfiguration(
            "La fuente devuelve HTML; requiere un conector autorizado".into(),
        ));
    }
    let etag = response
        .headers()
    ...
    let filename = response
        ...
        .map(safe_name)
        .unwrap_or_else(|| safe_name(request.filename));
    let (mut downloaded, total) = if response.status() == StatusCode::PARTIAL_CONTENT {
        ...
        if range.0 != offset
            || range.1 != range.2 - 1
            || (offset > 0 && etag.as_deref() != validator)
        {
            return Err(EmuBoxError::IpcError(
                "HTTP: rango o identidad del parcial no coinciden".into(),
            ));
        }
        (offset, Some(range.2))
    } else {
    ...
    let mut file = OpenOptions::new()
        ...
        .open(&partial)
        .map_err(io_error)?;
    let metadata = serde_json::to_vec(&ResumeIdentity {
    ...
    loop {
        ...
        let Some(chunk) = controlled(response.chunk(), request.control).await? else {
            file.sync_all().map_err(io_error)?;
            return Ok(TransferOutcome::Interrupted);
        };
        ...
        if downloaded == 0 {
            let prefix = String::from_utf8_lossy(&chunk[..chunk.len().min(512)])
                .trim_start()
                .to_ascii_lowercase();
            if prefix.starts_with("<!doctype html") || prefix.starts_with("<html") {
                return Err(EmuBoxError::InvalidConfiguration(
                    "Contenido HTML no descargable como juego".into(),
                ));
            }
        }
        file.write_all(&chunk).map_err(io_error)?;
        downloaded += chunk.len() as u64;
        if request.max_bytes.is_some_and(|limit| downloaded > limit) {
            return Err(EmuBoxError::InvalidConfiguration(
                "El descriptor supera el limite permitido".into(),
            ));
        }
        if total.is_some_and(|total| downloaded > total) {
            return Err(EmuBoxError::IpcError(
                "HTTP: contenido excede el tamano declarado".into(),
            ));
        }
    ...
    if total.is_some_and(|total| downloaded != total) || downloaded == 0 {
        return Err(EmuBoxError::IpcError(
            "HTTP: contenido vacio o incompleto".into(),
        ));
    }
    file.sync_all().map_err(io_error)?;
    drop(file);
...
mod tests {
    ...
    fn resume_checks_range_and_resource_identity() {
        for (index, range, etag, valid) in [
            (0, "bytes 4-7/8", "\"v1\"", true),
            (1, "bytes 0-3/8", "\"v1\"", false),
            (2, "bytes 4-7/8", "\"v2\"", false),
        ] {
            let server = TcpListener::bind("127.0.0.1:0").unwrap();
            let uri = format!("http://{}/content.bin", server.local_addr().unwrap());
            let worker = thread::spawn(move || {
            ...
            let root =
                std::env::temp_dir().join(format!("emubox-resume-{}-{index}", std::process::id()));
            fs::create_dir_all(&root).unwrap();
            fs::write(root.join("payload.part"), b"data").unwrap();
            fs::write(
                root.join("resume.json"),
                serde_json::to_vec(&ResumeIdentity {
                    uri: uri.clone(),
                    etag: Some("\"v1\"".into()),
                })
                .unwrap(),
            ...
            if valid {
                assert_eq!(fs::read(root.join("content.bin")).unwrap(), b"datamore");
            } else {
                assert!(!root.join("content.bin").exists());
                assert_eq!(fs::read(root.join("payload.part")).unwrap(), b"data");
            }
            worker.join().unwrap();
