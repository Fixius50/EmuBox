...
impl GameService {
    ...
    fn scan_directory_recursive(
        ...
        let ps4_folder = plat.id == "ps4" && dir.join("eboot.bin").is_file() && dir.join("sce_sys/param.sfo").is_file();
        if (plat.id == "ps3" && dir.join("PS3_GAME").is_dir()) || ps4_folder {
            *scanned += 1;
            ...
            let rom_path = if ps4_folder { dir.join("eboot.bin") } else { dir.to_path_buf() }.to_string_lossy().to_string();
...
mod tests {
    ...
    fn ps4_folder_registers_boot_file_once() {
        ...
        let result = GameService::scan_games(Some(ScanGamesRequest { platforms: Some(vec!["ps4".into()]), roms_directory: Some(root.to_string_lossy().into_owned()), deep_scan: Some(true) })).unwrap();
        assert_eq!(result.scanned_count, 1);
        let games = GameService::get_games(Some(GameFilter { platform: Some("ps4".into()), search: None, favorite: None, limit: None, offset: None })).unwrap();
        assert!(games.iter().any(|entry| entry.rom_path.as_deref() == game.join("eboot.bin").to_str()));
