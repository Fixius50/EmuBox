...
use crate::{
    errors::EmuBoxError,
    models::{
        DownloadJob, DownloadSource, DownloadStatus, ProviderId, PublishedDownload,
        TransferControl, TransferOutcome, TransferRequest,
    },
};
...
pub fn start(id: String) -> Result<DownloadJob, EmuBoxError> {
    ...
    let source_json: Option<String> = DatabaseService::get_connection()?
        .query_row(
...
fn run(
    ...
    if destination.exists() {
        let package: PublishedDownload = serde_json::from_slice(
    ...
    let (files, preparation_reason) =
        match super::download_preparation::prepare(&files, &root, control) {
            ...
            .enumerate()
        {
            ...
            if index == 0 {
                ...
                assert!(Path::new(&finished.destination_path)
                    ...
                    .is_file());
                assert!(
                    !super::super::GameService::get_game_by_id(game_id)
                        .unwrap()
                        .unwrap()
                        .installed
                );
