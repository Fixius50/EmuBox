...
export class TauriBackendService implements IEmuBoxBackend {
  ...
  public startGogAuthorization(): Promise<void> {
  ...
  public disconnectGog(): Promise<void> {
  ...
  public async selectJackettResult(gameId: string, resultId: string): Promise<DownloadSource> {
    return this.invoke('select_jackett_result', { gameId, resultId });
  }
  ...
  public async deleteCancelledDownload(id: string): Promise<void> {
    return this.invoke('delete_cancelled_download', { id });
  }

  public async uninstallGame(gameId: string): Promise<void> {
    return this.invoke('uninstall_game', { gameId });
  }
