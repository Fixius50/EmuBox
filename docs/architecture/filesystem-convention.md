...
- `/var/lib/emubox/stores` is owned by `emubox` with mode `0700`. Each provider must create its own session directory with mode `0700` before storing a credential or session artifact.
